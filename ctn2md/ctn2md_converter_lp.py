...
from ctn2md.src.ctn2md_gen_md_llamaparse import GenMd_LlamaParse
...
def _do_content2md_convert_core_lp(
    ...
    if md_info_path is None:
        logging.error(f"DCCCP: failed {md_info_path} @initial_gen_md_by_llamaparse")
        return None
    ...
    if md_generator.need_followup_step("fix_headings_plc_by_llm"):
        ...
        if md_info_path is None:
            logging.error(f"DCCCP: failed {md_info_path} @fix_headings_plc_by_llm")
            return None

    src_step_num += 1
    md_info_path = inject_section_heirarchy(md_info_path, src_step_num=src_step_num)
    if md_info_path is None:
        logging.error(f"DCCCP: failed {md_info_path} @inject_section_heirarchy")
        return None

    src_step_num += 1
    md_info_path = summarize_content_by_llm(md_info_path, src_step_num=src_step_num)
    if md_info_path is None:
        logging.error(f"DCCCP: failed {md_info_path} @summarize_content_by_llm")
        return None
...
def do_content2md_convert(
    ...
    try:
        pathname_final_json = _do_content2md_convert_core_lp(
            doc_pathname,
            out_dir=out_dir,
