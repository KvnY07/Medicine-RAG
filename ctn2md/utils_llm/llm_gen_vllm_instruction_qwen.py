...
from ctn2md.utils_llm.llm_gen_vllm_instruction_qwen_prompt import SYSTEM_PROMPT_GEN_VLLM_CONTEXT, USER_PROMPT_GEN_VLLM_CONTEXT
...
def gen_vllm_instruction_by_qwen(doc_title, page_content, model="qwen-turbo-1101", question_num=None):
    if question_num is None:
        question_num = get_next_question_num()

    try:
        if model not in ["qwen-plus", "qwen-plus-1127", "qwen-turbo-1101"]: 
...
1
 Restricted © SOTOS 2017. All Rights Reserved."""
    ...
    instruction= gen_vllm_instruction_by_qwen(doc_title, page_content)
